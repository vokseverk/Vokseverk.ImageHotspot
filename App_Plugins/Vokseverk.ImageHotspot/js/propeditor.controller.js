	...
	$scope.findImageReference = function (context, alias) {
		...
		while (context != null && !found && maxRecurse > 0) {
			...
			if (ref) {
				...
				if (props) {
					imageProperties = props.filter(function (prop) {
						return prop.alias.match(aliasRE);
					});

					if (imageProperties.length >= 1) {
						imageReference = imageProperties[0].value[0].mediaKey;
						found = true;
					}
				} else if (tabs) {
	...
	$scope.updateImageSrc = function (imageRef) {
		if (imageRef) {
			mediaResource.getById(imageRef).then(function (media) {
				...
				if (propsHolder) {
					var originalWidth = propsHolder.find(function (prop) {
						return prop.alias == "umbracoWidth";
					});
					var originalHeight = propsHolder.find(function (prop) {
						return prop.alias == "umbracoHeight";
					});
